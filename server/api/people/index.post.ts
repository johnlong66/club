...
export default defineEventHandler(async (event) => {
  ...
  const id = randomUUID()
  db.prepare('INSERT INTO people (id, prename, prenameUnknown, nickname, nicknameUnsure, meetingDay, comment) VALUES (?, ?, ?, ?, ?, ?, ?)')
    .run(id, prename ?? null, prenameUnknown ? 1 : 0, nickname ?? null, nicknameUnsure ? 1 : 0, meetingDay, comment ?? null)
